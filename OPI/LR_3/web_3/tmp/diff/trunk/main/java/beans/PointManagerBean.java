...
public class PointManagerBean implements Serializable {
    ...
    private final MISSProp missProp = new MISSProp();

    @PostConstruct
    public void init() {
        String sessionId = getSessionId();
        mBeanMan.registerBean(hitArea, "HitArea_" + sessionId);
        mBeanMan.registerBean(missProp, "MISSProp_" + sessionId);
        logger.info("MBeans registered for session: " + sessionId);
    }

    @PreDestroy
    public void destroy() {
        mBeanMan.unregisterBean(hitArea);
        mBeanMan.unregisterBean(missProp);
    ...
    public void savePoint(PointBean pointBean) {
        ...
        try (Connection connection = DbManager.getConnection();
            ...
            hitArea.updateStats(isHit);
            missProp.updateStats(isHit);
